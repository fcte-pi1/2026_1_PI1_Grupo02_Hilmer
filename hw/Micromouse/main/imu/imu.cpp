...
// O bloco extern "C" evita name mangling da biblioteca em C.[cite: 7]
extern "C" {
...
static i2c_master_bus_handle_t i2c_bus_handle; // Handle do barramento I2C[cite: 7]
...
static void salvar_calibracao_na_nvs() {
    ...
    // Grava a struct inteira como blob para reduzir tempo e desgaste da flash.[cite: 7]
    if (nvs_set_blob(nvs_handle, NVS_BLOB_KEY, &calibracao_atual, sizeof(CalibracaoIMU)) == ESP_OK) {
        nvs_commit(nvs_handle); // Efetiva a gravacao no chip.[cite: 7]
...
bool imu_update() {
    ...
    dados_atuais.timestamp_ms = esp_timer_get_time() / 1000; // Tempo desde o boot em ms.[cite: 7]
...
bool imu_calibrar_gyro() {
    ...
    for (int i = 0; i < amostras; i++) {
        ...
        vTaskDelay(pdMS_TO_TICKS(5)); // Cede CPU ao RTOS.[cite: 7]
    }
